...
local function hud()
	...
	draw.SimpleText(plyhealth , "lukefont", w/2-5, h-32, Color(0,0,255)) // text
end
...
concommand.Add( "luke_gui", function ()
	...
	Frame.Paint = function( self, w, h ) -- 'function Frame:Paint( w, h )' works too
		draw.RoundedBox( 0, 0, 0, w, h, Color( 73, 86, 116, 200 ) ) -- Draw a red box instead of the frame
	end
	...
	CheckBoxThing:SetValue( GetConVar( "luke_hud" ):GetString() )
	CheckBoxThing:SizeToContents() -- Make its size to the contents. Duh?

	local CheckBoxThing2 = vgui.Create( "DCheckBoxLabel", Frame )
	...
	CheckBoxThing2:SetValue( GetConVar( "luke_bhop" ):GetString() )
	CheckBoxThing2:SizeToContents() -- Make its size to the contents. Duh?

	local CheckBoxThing5 = vgui.Create( "DCheckBoxLabel", Frame )
	CheckBoxThing5:SetPos( 10, 90 )
	CheckBoxThing5:SetText( "Enable RapidFire?" )
	CheckBoxThing5:SetConVar( "luke_strafe" ) -- ConCommand must be a 1 or 0 value
	CheckBoxThing5:SetValue( GetConVar( "luke_strafe" ):GetString() )
	CheckBoxThing5:SizeToContents() -- Make its size to the contents. Duh?

	local CheckBoxThing3 = vgui.Create( "DCheckBoxLabel", Frame )
	...
	CheckBoxThing3:SetValue( GetConVar( "luke_esp" ):GetString() )
	CheckBoxThing3:SizeToContents() -- Make its size to the contents. Duh?

	local CheckBoxThing4 = vgui.Create( "DCheckBoxLabel", Frame )
	CheckBoxThing4:SetPos( 10, 90 )
	CheckBoxThing4:SetText( "Enable RapidFire?" )
	CheckBoxThing4:SetConVar( "luke_rapidfire" ) -- ConCommand must be a 1 or 0 value
	CheckBoxThing4:SetValue( GetConVar( "luke_rapidfire" ):GetString() )
	...
	CheckBoxThing5:SetValue( GetConVar( "luke_walls" ):GetString() )
	CheckBoxThing5:SizeToContents() -- Make its size to the contents. Duh?
end )
-------------------------------------------------------------------------
...
if CLIENT then // we don't want to reveal ourselves do we?
	luke.rainbow("------------------------\n")
